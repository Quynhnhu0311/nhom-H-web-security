        ...
        <?php if ($post || !isset($post_id)) { ?>
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" name="post_description" placeholder="Description" value='<?php if (!empty($post[0]['post_description'])) echo $post[0]['post_description'] ?>'>
                </div>
                <div class="form-group">
                    <?php foreach ($usersid as $key) { ?>
                        <label for="token">Token</label>
                        <input type="password" name="token" class="form-control" value='<?php echo md5($_SESSION['id'] . $key['key_user']); ?>'>
                    <?php } ?>
                </div>
